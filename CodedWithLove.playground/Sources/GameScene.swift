...
public class GameSceneFile: SKScene, SKPhysicsContactDelegate {
    ...
    override public func update(_ currentTime: TimeInterval) {
        if !isPlaying && asteroidsNumber == 0 {
            ...
            run(SKAction.sequence([SKAction.sequence([SKAction.wait(forDuration: 1), SKAction.playSoundFileNamed("sounds/sfx/phaserUp6.mp3" , waitForCompletion: false)])]))
            aMessage.text = "3"
        ...
        enumerateChildNodes(withName: "asteroid*") {
            ...
            let baseSpeed: CGFloat = node.name == "asteroidBig" ? 60 : 110
            let xSpeed: CGFloat = (baseSpeed + (CGFloat(self.gameLevel) * 8))
            let ySpeed: CGFloat = (baseSpeed + (CGFloat(self.gameLevel) * 8))
    ...
    func addPowerUp(inPosition: CGPoint) {
        ...
        powerUp.run(SKAction.sequence([SKAction.fadeIn(withDuration: 0.5), SKAction.repeatForever(SKAction.sequence([SKAction.scale(by: 1.1, duration: 0.5), SKAction.scale(by: 0.9, duration: 0.5)]))]))
        addChild(powerUp)
    }
